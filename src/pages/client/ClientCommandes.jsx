...
const ClientCommandes = () => {
    ...
    const STATUT_CHOICES = [
        ...
        { value: 'EN_ATTENTE', label: 'En attente', icon: <FaClock />, color: 'yellow' },
        { value: 'EN_COURS', label: 'En cours', icon: <FaTruck />, color: 'blue' },
        { value: 'PRET', label: 'Prêt', icon: <FaCheckCircle />, color: 'green' },
        { value: 'LIVRE', label: 'Livré', icon: <FaCheckCircle />, color: 'purple' },
        { value: 'ANNULE', label: 'Annulé', icon: <FaExclamationCircle />, color: 'red' },
    ];
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div className="mt-4 text-sm text-gray-600">
                        <span className="font-semibold">{filteredCommandes.length}</span> commande(s)
                        trouvée(s)
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                    {STATUT_CHOICES.slice(1).map((statut) => {
                        const count = commandes.filter((c) => c.statut === statut.value).length;
                        const colorClasses = {
                        ...
                        return (
                            <button
                                ...
                                className={`bg-white rounded-xl shadow-lg p-4 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 ${filterStatut === statut.value ? 'ring-4 ring-blue-500' : ''
                                    }`}
                            >
                                <div
                                    className={`w-12 h-12 bg-gradient-to-br ${colorClasses[statut.color]
                                        } rounded-full flex items-center justify-center mx-auto mb-3`}
                                >
                                    <span className="text-white text-xl">{statut.icon}</span>
                                </div>
                                <p className="text-2xl font-bold text-gray-900">{count}</p>
                                <p className="text-xs text-gray-600 mt-1">{statut.label}</p>
                            </button>
                        );
                    })}
                </div>
                    ...
                    <div className="space-y-4">
                        {filteredCommandes.map((commande) => {
                            ...
                            return (
                                ...
                                >
                                    {/* Status Bar */}
                                    <div className={`h-2 bg-gradient-to-r ${statutInfo.gradient}`}></div>
                                        ...
                                        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-4 lg:space-y-0">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-600">
                                                    <div className="flex items-center space-x-2">
                                                        ...
                                                        <span>
                                                            Déposé le{' '}
                                                            {format(new Date(commande.date_depot), 'dd MMMM yyyy à HH:mm', {
                                                                locale: fr,
                                                            })}
                                                        </span>
                                                    </div>
                                                    <div className="flex items-center space-x-2">
                                                        ...
                                                        <span>
                                                            Promis le{' '}
                                                            {format(
                                                                new Date(commande.date_promesse),
                                                    ...
                                                    {commande.date_retrait && (
                                                        <div className="flex items-center space-x-2">
                                                            ...
                                                            <span>
                                                                Retiré le{' '}
                                                                {format(
                                                                    new Date(commande.date_retrait),
                                                                    'dd MMMM yyyy',
                                                                    { locale: fr }
                                                                )}
                                                            </span>
                                            ...
                                            <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-6 lg:ml-6">
                                                <div className="text-right">
                                                    ...
                                                    {parseFloat(commande.montant_restant) > 0 ? (
                                                        <p className="text-sm text-red-600 font-semibold mt-1">
                                                            Reste à payer:{' '}
                                                            {parseFloat(commande.montant_restant).toLocaleString()} FCFA
                                                        </p>
                                                    ) : (
                                                ...
                                                <button
                                                    ...
                                                    className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-lg font-semibold hover:from-blue-700 hover:to-purple-700 transition-all flex items-center space-x-2 shadow-lg hover:shadow-xl"
                                                >
                                                    <FaEye />
                                                    <span>Détails</span>
                                                </button>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-6">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span
                                        className={`inline-flex items-center space-x-2 px-4 py-2 rounded-full text-sm font-semibold border-2 ${getStatutInfo(selectedCommande.statut).color
                                            }`}
                                    >
                                        {getStatutInfo(selectedCommande.statut).icon}
                                        <span>{getStatutInfo(selectedCommande.statut).label}</span>
                                    </span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-4">
                                    <TimelineItem
                                        icon={<FaCalendarAlt />}
                                        label="Déposé"
                                        date={selectedCommande.date_depot}
                                        completed={true}
                                    />
                                    <TimelineItem
                                        icon={<FaTruck />}
                                    ...
                                    <TimelineItem
                                        icon={<FaCheckCircle />}
                                        label="Prêt"
                                        date={selectedCommande.date_promesse}
                                        completed={['PRET', 'LIVRE'].includes(selectedCommande.statut)}
                                    />
                                    {selectedCommande.date_retrait && (
                                        <TimelineItem
                                            icon={<FaCheckCircle />}
                                            label="Retiré"
                                            date={selectedCommande.date_retrait}
                                            completed={true}
                                        />
                                    )}
                                </div>
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {selectedCommande.articles?.map((article) => (
                                                <tr key={article.id}>
                                                    ...
                                                    <td className="px-6 py-4 text-sm text-gray-900">
                                                        {parseFloat(article.prix_unitaire).toLocaleString()} FCFA
                                                    </td>
                                                    <td className="px-6 py-4 text-sm font-semibold text-gray-900">
                                                        {parseFloat(article.prix_total).toLocaleString()} FCFA
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between text-gray-700">
                                        ...
                                        <span className="font-semibold">
                                            {parseFloat(selectedCommande.montant_total).toLocaleString()} FCFA
                                        </span>
                                    </div>
                                    <div className="flex justify-between text-gray-700">
                                        <span>Montant payé:</span>
                                        <span className="font-semibold text-green-600">
                                            {parseFloat(selectedCommande.montant_paye).toLocaleString()} FCFA
                                        </span>
                                    </div>
                                    <div className="flex justify-between text-lg font-bold text-gray-900 border-t pt-2">
                                        ...
                                        <span
                                            className={
                                                parseFloat(selectedCommande.montant_restant) > 0
                                                    ? 'text-red-600'
                                                    : 'text-green-600'
                                            }
                                        >
                                            {parseFloat(selectedCommande.montant_restant).toLocaleString()} FCFA
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            {selectedCommande.notes && (
                                <div className="bg-blue-50 rounded-xl p-4">
                                    ...
                                    <p className="text-sm text-blue-800">{selectedCommande.notes}</p>
                                </div>
                            )}
                        </div>
                    )}
