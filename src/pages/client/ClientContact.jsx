...
const ClientContact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.last_name || !formData.message) {
      setAlert({
        type: "error",
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            {contactInfo.map((item, index) => (
              <a
                key={index}
                href={item.action || "#"}
                className={`block bg-white rounded-xl shadow-lg p-6 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 ${
