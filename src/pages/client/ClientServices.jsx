...
import { publicServiceService, publicCategorieService } from '../../services/clientApi';
...
const ClientServices = () => {
    ...
    const filterServices = () => {
        ...
        if (selectedCategorie) {
            filtered = filtered.filter((s) => s.categorie === parseInt(selectedCategorie));
        }
            ...
            <div className="space-y-8">
                ...
                <div className="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div className="mt-4 text-sm text-gray-600">
                        <span className="font-semibold">{filteredServices.length}</span> service(s)
                        disponible(s)
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                    <button
                        ...
                        className={`bg-white rounded-xl shadow-lg p-6 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 ${selectedCategorie === '' ? 'ring-4 ring-blue-500' : ''
                            }`}
                    >
                    ...
                    {categories.map((cat, index) => {
                        const count = services.filter((s) => s.categorie === cat.id).length;
                        return (
                            <button
                                key={cat.id}
                                onClick={() => setSelectedCategorie(cat.id.toString())}
                                className={`bg-white rounded-xl shadow-lg p-6 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 ${selectedCategorie === cat.id.toString() ? 'ring-4 ring-blue-500' : ''
                                    }`}
                            >
                                <div
                                ...
                                <p className="text-2xl font-bold text-center text-gray-900 mt-2">{count}</p>
                            </button>
                        );
                    })}
                </div>
                ...
                {filteredServices.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredServices.map((service) => {
                            const category = categories.find((c) => c.id === service.categorie);
                            return (
                                <div
                                    key={service.id}
                                ...
                                >
                                    ...
                                    <div
                                        className={`h-2 bg-linear-to-r ${getCategoryColor(
                                            categories.findIndex((c) => c.id === service.categorie)
                                        )}`}
                                    ></div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex items-center justify-between bg-linear-to-r from-green-50 to-emerald-50 rounded-lg p-3">
                                                ...
                                                <span className="text-2xl font-bold bg-gr-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                                                    {parseFloat(service.prix).toLocaleString()} FCFA
                                                </span>
                                            </div>
